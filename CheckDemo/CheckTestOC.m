...
#pragma mark 注入攻击风险(适用于主工程app)
    ...
    // 注意⚠️: 当前是判断主工程可执行文件macho文件对应的链接库列表, 所以传的是主工程对应名称
    // 如果是动态库的防护代码，则添加动态库名称如TestFramework.framework/TestFramework
    if (IJMCheckMachoInsertList("CheckDemo.app/CheckDemo"))
    {
        NSLog(@"检测到动态库注入");
        exit(0);
    }
    
#pragma mark 注入攻击风险(适用于动态库)
//    // 注意⚠️: 当前是判断主工程可执行文件macho文件对应的链接库列表, 所以传的是主工程对应名称
//    // 如果是动态库的防护代码，则添加动态库名称如TestFramework.framework/TestFramework
//    if (IJMCheckMachoInsertList("TestFramework.framework/TestFramework"))
//    {
//        NSLog(@"检测到动态库注入");
//        exit(0);
//    }
...
#warning 这里调试时开启日志打印，正式发布时关闭日志打印，防止泄露特征位置
...
// 动态库注入攻击风险
// 这种方式是通过检测macho文件的链接库列表，来判断有没有被注入
// 非越狱环境下trollFools注入可通过此方式防护
// 所以需要预先知道macho文件的链接库，可以通过编译完成后的文件，执行otool -L <macho文件>得到
// 也可以通过下面的日志打印得到
#define MJEndianConvert(big, value) \
((big) ? OSSwapInt32((value)) : (value))
#warning 在这里修改动态库列表，如果不清楚列表内容，可以通过看下面的代码打印数据后再进行修改
static const char *macholibs[] = {
...
__attribute__((always_inline)) static bool IJMCheckMachoInsertList(char *machoname)
{
    // 得到匹配的文件镜像
    ...
    unsigned long pFile = (unsigned long)header;
    int headerLength = sizeof(struct mach_header);
    BOOL bigEndian = (header->magic == MH_CIGAM);
    BOOL is64bit = NO;
    if (header->magic == MH_MAGIC_64 || header->magic == MH_CIGAM_64) {
        headerLength = sizeof(struct mach_header_64);
        bigEndian = (header->magic == MH_CIGAM_64);
        is64bit = YES;
    }
    
    unsigned long currentAddress = pFile + headerLength;
    // lc的数量
    ...
    {
        struct load_command *lc = (struct load_command  *)currentAddress;
...
#warning 这里调试时开启日志打印，正式发布时关闭日志打印，防止泄露特征位置
            NSLog(@"需要添加到_macholibs的链接库名称:%s", dylibname);
            
            bool containlib = false;
            int arrayLength = sizeof(macholibs) / sizeof(macholibs[0]);
            for (int i = 0; i < arrayLength; ++i)
            {
                if (strncmp(dylibname, macholibs[i], strlen(macholibs[i])) == 0)
                {
                    containlib = true;
                    break;
                }
            ...
            {
#warning 这里调试时开启日志打印，正式发布时关闭日志打印，防止泄露特征位置
                NSLog(@">>>>>dylibname = %s", dylibname);
                return YES;
            }
        }
        
        currentAddress += lc->cmdsize;
    }
    return NO; // 如果所有动态库均在白名单中，则返回 NO，表示所有动态库都在白名单内
}
